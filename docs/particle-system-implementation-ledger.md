...
- Second follow-up: recipe browsing/categorization is closed. Recipes are ordinary `ParticleEffect` assets, so a dedicated recipe UI would have duplicated the Asset Browser; instead `recipeCategory` (already validated and round-tripped since Stage 1B) is now surfaced there. A full `AssetManager::Load<T>()` per row was rejected — it unconditionally reloads from disk and would force-parse every recipe on every browser refresh/keystroke, a real hot-path cost, not a cosmetic one. Chose instead a new optional `IAssetLoader::DiscoverImportCategory(path)` hook (default empty, mirroring the existing `DiscoverDependencies`/`ValidateDependencies` no-op-default pattern), called once per file during `AssetDiscoveryService::DiscoverMountedAssets` — the same place the pre-existing `ImportedAsset` header-category read already runs, and not a per-frame path. `ParticleEffectAssetLoader` overrides it with one `LoadDetailed` call and returns `recipeCategory`; every other loader keeps the free default. The result lands in `AssetMetadata.importCategory`, a field the Asset Browser's `SearchText()` (`EditorAssetBrowserAssetRows.cpp:38`) already folds into search — so browsing by category ("Hit Impact", "Trail", "Aura", ...) now works with zero further browser-side changes. A dedicated dependency-navigation test remains open; see Rejected attempts, open defects.
...
- Recipe browsing follow-up touches `sources/engine/include/engine/assets/IAssetLoader.hpp` (`DiscoverImportCategory` hook), `sources/engine/include/engine/scene/ParticleEffectAssetLoader.hpp`/`.cpp` (override), `sources/engine/src/assets/AssetDiscoveryService.cpp` (wiring into the existing discovery scan), and `sources/engine/tests/ParticleEffectAssetTests.cpp` (`RunRecipeAssetTest` extended to assert the discovered category for all 15 recipes).
