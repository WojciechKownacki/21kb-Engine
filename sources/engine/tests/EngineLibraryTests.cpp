...
void RunModuleInstallReportsDuplicateDiagnosticsTest() {
    ...
    kb::tests::Require(second.diagnostics.size() == 18U, "Engine21kbLibrary module install must report one diagnostic per failed domain module");
}
...
void RunModuleCatalogTest() {
    ...
    const std::vector<std::string> expectedNames{ "Input", "Audio", "World", "Time", "Timer", "Task", "Physics", "Transform", "Math", "Scene", "MeshRenderer", "MaterialInstance", "PostProcess", "Particles", "Renderer", "Assets", "Save", "Collections" };
    kb::tests::Require(catalog.size() == expectedNames.size(), "Engine21kbLibrary module catalog must have exactly eighteen domain modules");
