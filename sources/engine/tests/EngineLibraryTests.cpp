...
void RunGameInstanceLifetimeTest() {
    ...
    kb::core::EngineLog engineLog{ 5U }; const kb::core::LogRecord logRecord{ .category="AI", .message="event", .entity=1U, .world=2U, .fields={{"state","alert"}} }; kb::tests::Require(engineLog.Trace(logRecord, 1U, 1U) && engineLog.Debug(logRecord, 2U, 1U) && engineLog.Info(logRecord, 3U, 1U) && engineLog.Warn(logRecord, 4U, 1U) && engineLog.Error(logRecord, 5U, 1U) && !engineLog.Warn(logRecord, 4U, 1U) && engineLog.Records().size()==5U && engineLog.Records().front().level==kb::core::LogLevel::Trace && engineLog.Records().back().level==kb::core::LogLevel::Error && engineLog.Records().front().entity==1U && engineLog.Records().front().world==2U && engineLog.Records().front().fields.front().key=="state" && !kb::core::EngineLog{0U}.Info(logRecord,1U,1U), "Engine log did not retain levels, structured context or rate limit duplicate keys");
