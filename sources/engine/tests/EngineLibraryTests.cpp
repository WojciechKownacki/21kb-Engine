...
#include "engine/core/BindingCache.hpp"
...
void RunModuleInstallReportsDuplicateDiagnosticsTest() {
    ...
    kb::tests::Require(second.diagnostics.size() == 23U, "Engine21kbLibrary module install must report one diagnostic per failed domain module");
}
...
void RunModuleCatalogTest() {
    ...
    const std::vector<std::string> expectedNames{ "Input", "Audio", "World", "Time", "Timer", "Task", "Events", "Physics", "Transform", "Math", "Scene", "MeshRenderer", "MaterialInstance", "PostProcess", "Particles", "Animator", "Timeline", "Localization", "Renderer", "Assets", "Save", "Collections", "Text" };
...
void RunGameInstanceLifetimeTest() {
    ...
    kb::tests::Require(
        transformBinding != kb::core::kInvalidBindingId
            && transformBinding == bindings.Register("Transform")
            && bindings.Find("Transform") == transformBinding
    ...
    kb::tests::Require(sampleProfiles[0].kind == kb::gameplay::GameplaySampleKind::ThirdPerson && sampleProfiles[0].usesJump && sampleProfiles[1].movement == kb::gameplay::GameplaySampleMovement::ScreenPlane && sampleProfiles[2].movement == kb::gameplay::GameplaySampleMovement::SideScroll && sampleProfiles[3].cameraPolicy == kb::gameplay::CameraPolicy::Possess && sampleProfiles[3].usesCombat && sampleProfiles[4].kind == kb::gameplay::GameplaySampleKind::AudioShooter && sampleProfiles[4].movement == kb::gameplay::GameplaySampleMovement::ForwardFlight && sampleProfiles[4].usesCombat && sampleProfiles[4].usesAudio, "Gameplay samples did not provide the five documented controller profiles");
