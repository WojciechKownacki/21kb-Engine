...
[[nodiscard]] inline bool ValidateIncomingMessage(const NetworkObjects& objects, NetworkSecurityLimits limits, NetworkObjectId object, NetworkPeerId sender, std::size_t payloadBytes, std::uint32_t messagesThisTick, std::size_t declaredBytes) noexcept { return IsValidSecurityLimits(limits)&&object!=0U&&sender!=0U&&payloadBytes<=limits.maxPayloadBytes&&declaredBytes==payloadBytes&&messagesThisTick<limits.maxMessagesPerTick&&objects.CanAcceptOwnerCommand(object,sender); }
} // namespace kb::network
