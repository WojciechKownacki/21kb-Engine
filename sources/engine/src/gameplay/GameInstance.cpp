...
bool GameInstance::DestroyScene(GameSceneId id) noexcept { const auto it = std::find_if(scenes_.begin(), scenes_.end(), [id](const Entry& entry){ return entry.id == id; }); if (it == scenes_.end()) return false; scenes_.erase(it); if (activeScene_ == id) activeScene_ = scenes_.empty() ? 0U : scenes_.front().id; return true; }
