...
std::optional<GameSceneId> GameFlow::Restart() noexcept { if(!checkpoint_.has_value() || (state_ != GameFlowState::Won && state_ != GameFlowState::Lost))return std::nullopt; state_=GameFlowState::Transitioning; pendingTransition_=checkpoint_; return pendingTransition_; }
bool GameFlow::BeginTransition(GameSceneId destination) noexcept { if(destination==0U||state_!=GameFlowState::Playing)return false; pendingTransition_=destination; state_=GameFlowState::Transitioning; return true; }
