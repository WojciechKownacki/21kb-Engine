...
bool ReadValue(Reader& in, UISelectable& v, std::uint32_t fileVersion) {
    ...
    return fileVersion < 38U || in.ReadUInt64(v.targetGraphic);
}
...
void WriteValue(std::vector<std::uint8_t>& out, const UIScrollView& v) { SceneAssetBinaryIO::WriteFloat(out,v.scrollX);SceneAssetBinaryIO::WriteFloat(out,v.scrollY);SceneAssetBinaryIO::WriteFloat(out,v.scrollSensitivity);SceneAssetBinaryIO::WriteBool(out,v.horizontal);SceneAssetBinaryIO::WriteBool(out,v.vertical);SceneAssetBinaryIO::WriteBool(out,v.inertia);SceneAssetBinaryIO::WriteUInt64(out,v.verticalScrollbar); }
