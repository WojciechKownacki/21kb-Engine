...
std::optional<SkeletalMeshAsset> SkeletalMeshAssetIO::Load(
    ...
    while (std::getline(file, line)) {
        ...
        else if (cmd == "boneBounds") { std::size_t l=0; SkeletalMeshBoneBounds b{}; if (!(in>>l>>b.boneId>>b.center.x>>b.center.y>>b.center.z>>b.extents.x>>b.extents.y>>b.extents.z)||l>=asset.lods.size()||!End(in)) return std::nullopt; asset.lods[l].boneBounds.push_back(b); }
