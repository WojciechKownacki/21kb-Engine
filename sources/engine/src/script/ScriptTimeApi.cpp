...
ScriptFunctionCallResult FixedDelta(const ScriptFunctionCallContext& context, std::span<const ScriptFunctionArgument>) {
    ...
    // Reports the delta FixedTick actually receives. For an installed scene
    // system this is the same authoritative fixed delta physics consumes;
    // direct/manual ExecuteFrame keeps its explicit harness delta.
    return ScriptFunctionCallResult{
        .executed = true,
