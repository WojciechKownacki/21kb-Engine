...
UserWidgetBackgroundBlur::~UserWidgetBackgroundBlur() {
    Shutdown();
}

bool UserWidgetBackgroundBlur::Initialize() {
    if (IsInitialized()) {
        return true;
    }
...
void UserWidgetBackgroundBlur::Shutdown() noexcept {
    InvalidateTargets();
...
void UserWidgetBackgroundBlur::InvalidateTargets() noexcept {
    for (TargetSet& targets : targets_) {
        Release(targets);
    }
}

bool UserWidgetBackgroundBlur::Submit(std::uint32_t viewportIndex, RenderExtent extent,
    ...
    if (!SubmitPass(horizontalView, extent, targets.pingFrameBuffer, source,
                    sampleRadius / static_cast<float>(extent.width), 0.0F, "KB Runtime UI Blur H")) {
        return false;
    }
    return SubmitPass(verticalView, extent, targets.outputFrameBuffer, targets.pingTexture, 0.0F,
                      sampleRadius / static_cast<float>(extent.height), "KB Runtime UI Blur V");
}

bgfx::TextureHandle UserWidgetBackgroundBlur::Output(std::uint32_t viewportIndex) const noexcept {
    if (viewportIndex < targets_.size()) {
        return targets_[viewportIndex].outputTexture;
    }
    return BGFX_INVALID_HANDLE;
}

bool UserWidgetBackgroundBlur::IsInitialized() const noexcept {
    return bgfx::isValid(program_) && bgfx::isValid(sourceSampler_) && bgfx::isValid(paramsUniform_);
}

bool UserWidgetBackgroundBlur::Ensure(std::uint32_t viewportIndex, RenderExtent extent,
                                      bgfx::TextureFormat::Enum format) {
    if (viewportIndex >= targets_.size() || !extent.IsValid()) {
        return false;
    }
    ...
    Release(targets);
    if (extent.width > UINT16_MAX || extent.height > UINT16_MAX) {
        return false;
    }
    ...
    bgfx::setName(targets.pingTexture, "KB Runtime UI Blur Ping");
    bgfx::setName(targets.outputTexture, "KB Runtime UI Blurred");
...
void UserWidgetBackgroundBlur::Release(TargetSet& targets) noexcept {
...
bool UserWidgetBackgroundBlur::SubmitPass(std::uint16_t viewId, RenderExtent extent, bgfx::FrameBufferHandle output,
    ...
    bgfx::setViewRect(viewId, 0U, 0U, static_cast<std::uint16_t>(extent.width),
