...
[[nodiscard]] bool ValidateSimpleGlyph(std::span<const std::byte> glyph, std::uint16_t contours) {
    ...
    std::size_t contourStart = 0U;
    for (const std::uint16_t endpoint : endpoints) {
        const std::size_t contourEnd = static_cast<std::size_t>(endpoint);
        if (contourEnd == contourStart && (flags[contourStart] & 0x01U) == 0U)
            return false;
        contourStart = contourEnd + 1U;
    }
...
bool UserWidgetFontPayloadValidator::SupportsExtension(std::string_view extension) noexcept {
    return extension == ".ttf" || extension == ".otf";
}

bool UserWidgetFontPayloadValidator::Validate(const kb::assets::ImportedAsset& asset) {
    ...
    std::uint16_t tableCount = 0U;
    if (!ReadU32(bytes, 0U, version) || version != 0x00010000U || !ReadU16(bytes, 4U, tableCount) || tableCount == 0U ||
        tableCount > 128U || !RangeFits(12U, static_cast<std::size_t>(tableCount) * 16U, bytes.size()))
        return false;
    ...
    for (std::uint16_t index = 0U; index < tableCount; ++index) {
        ...
        if (!ReadU32(bytes, record, tag) || !ReadU32(bytes, record + 8U, offset) ||
            ...
            !RangeFits(offset, length, bytes.size()) || std::ranges::find(tables, tag, &FontTable::tag) != tables.end())
            return false;
