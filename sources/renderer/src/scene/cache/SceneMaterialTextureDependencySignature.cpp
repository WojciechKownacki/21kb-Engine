...
[[nodiscard]] std::array<MaterialTextureDependencySlot, 13U> TextureSlots(const RenderMaterialResource& material) noexcept {
    return {{
        MaterialTextureDependencySlot{ .directHandle = material.albedoTexture, .assetId = material.albedoTextureAssetId },
        MaterialTextureDependencySlot{ .directHandle = material.normalTexture, .assetId = material.normalTextureAssetId },
        MaterialTextureDependencySlot{ .directHandle = material.metallicRoughnessTexture, .assetId = material.metallicRoughnessTextureAssetId },
        MaterialTextureDependencySlot{ .directHandle = material.occlusionTexture, .assetId = material.occlusionTextureAssetId },
        MaterialTextureDependencySlot{ .directHandle = material.emissiveTexture, .assetId = material.emissiveTextureAssetId },
...
[[nodiscard]] RenderTextureHandle ResolveTextureHandle(
    ...
    return resourceMap == nullptr || slot.assetId == 0U ? RenderTextureHandle{} : resourceMap->ResolveTexture(slot.assetId);
}
