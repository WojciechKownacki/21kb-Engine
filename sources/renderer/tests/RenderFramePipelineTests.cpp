...
void RenderPassGraphValidatesRequiredPasses() {
    ...
    Require(graph.FindResource(RenderGraphResource::ScreenUIBlurPing) != nullptr, "RenderPassGraph did not declare screen UI blur ping");
    Require(graph.FindResource(RenderGraphResource::ScreenUIBlurred) != nullptr, "RenderPassGraph did not declare screen UI blurred output");
...
void PrimaryViewportIdsUseCanonicalRuntimeViews() {
    ...
    Require(ids.screenUIBlurH == ViewId::ScreenUIBlurH, "Primary screen UI horizontal blur view id is not canonical");
    Require(ids.screenUIBlurV == ViewId::ScreenUIBlurV, "Primary screen UI vertical blur view id is not canonical");
    Require(ids.finalComposite == ViewId::FinalComposite, "Primary final composite view id is not canonical");
    Require(ids.screenUIComposite == ViewId::ScreenUIComposite, "Primary screen UI composite view id is not canonical");
...
void DetachedViewportIdsAreContiguousAndBounded() {
    ...
    Require(first.screenUIBlurH == ViewId::DetachedViewportStart + 33U, "First detached viewport screen UI horizontal blur view id is wrong");
    Require(first.screenUIBlurV == ViewId::DetachedViewportStart + 34U, "First detached viewport screen UI vertical blur view id is wrong");
    Require(first.screenUIComposite == ViewId::DetachedViewportStart + 35U, "First detached viewport screen UI composite view id is wrong");
...
void FramePipelineBuildsCanonicalPassOrder() {
    ...
    constexpr std::array<RenderPassKind, RenderPassKindCount> expectedPasses{
        ...
        RenderPassKind::ScreenUIBlurH,
        RenderPassKind::ScreenUIBlurV,
        RenderPassKind::FinalComposite,
        RenderPassKind::ScreenUIComposite,
    ...
    constexpr std::array<std::uint16_t, RenderPassKindCount + (RenderViewportViewIds::kBloomPyramidExtraMipCount * 3U)> expectedViewOrder{
        ...
        ViewId::ScreenUIBlurH,
        ViewId::ScreenUIBlurV,
        ViewId::FinalComposite,
        ViewId::ScreenUIComposite,
        ViewId::EditorUi,
...
void FrameStateAccumulatesMultipleViewportViewOrders() {
    ...
    constexpr std::array<std::uint16_t, 73U> expectedViewOrder{
        ...
        ViewId::DetachedViewportStart + 35U,
