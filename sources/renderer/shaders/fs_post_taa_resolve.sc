...
void main()
{
    vec4 current = texture2D(s_source, v_texcoord0);
    if (u_temporalParams.y < 0.5) {
        gl_FragColor = current;
        return;
    }

    vec2 velocity = texture2D(s_velocity, v_texcoord0).xy * 2.0 - 1.0;
    vec2 historyUv = v_texcoord0 - velocity;
    ...
    vec3 c00 = texture2D(s_source, v_texcoord0 + vec2(-1.0, -1.0) * texel).rgb;
    vec3 c10 = texture2D(s_source, v_texcoord0 + vec2( 0.0, -1.0) * texel).rgb;
    vec3 c20 = texture2D(s_source, v_texcoord0 + vec2( 1.0, -1.0) * texel).rgb;
    vec3 c01 = texture2D(s_source, v_texcoord0 + vec2(-1.0,  0.0) * texel).rgb;
    vec3 c21 = texture2D(s_source, v_texcoord0 + vec2( 1.0,  0.0) * texel).rgb;
    vec3 c02 = texture2D(s_source, v_texcoord0 + vec2(-1.0,  1.0) * texel).rgb;
    vec3 c12 = texture2D(s_source, v_texcoord0 + vec2( 0.0,  1.0) * texel).rgb;
    vec3 c22 = texture2D(s_source, v_texcoord0 + vec2( 1.0,  1.0) * texel).rgb;
    ...
    blend *= 1.0 - saturate(pixelVelocity * 0.04);
    ...
    blend *= 1.0 - (saturate(lumDelta) * 0.35);
