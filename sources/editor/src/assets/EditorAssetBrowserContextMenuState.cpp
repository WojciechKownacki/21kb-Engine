...
std::vector<EditorAssetContextMenuItem> EditorAssetBrowserContextMenuState::Items(bool targetAssetExists, bool targetFolderCanMutate) const {
    ...
    case EditorAssetContextTargetKind::Folder:
        ...
        items.push_back(EditorAssetContextMenuItem{ .command = EditorAssetContextCommand::NewParticleEffect, .label = "New Particle Effect" });
        items.push_back(EditorAssetContextMenuItem{ .command = EditorAssetContextCommand::NewUserWidget, .label = "New User Widget", .separatorAfter = true });
