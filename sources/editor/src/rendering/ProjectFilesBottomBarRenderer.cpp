...
void DrawSortMenuItem(HDC dc, RECT rect, const EditorTheme& theme, const char* label, bool selected) {
    GdiDrawing::DrawSharpFrame(dc, rect, selected ? Draw::Color(theme.tabActive) : Draw::Color(theme.panel), Draw::Color(theme.borderPanel));
    Draw::DrawLabel(dc, Draw::Inset(rect, 8, 0), label, selected ? Draw::Color(theme.accent) : Draw::Color(theme.textPrimary));
}
...
void ProjectFilesBottomBarRenderer::Paint(HDC dc, const EditorAssetBrowserLayoutRects& layout, const EditorTheme& theme, const EditorAssetBrowserState& state) {
    GdiDrawing::DrawSharpFrame(dc, layout.bottomBar, Draw::Color(theme.strip), Draw::Color(theme.borderPanel));
    ...
    GdiDrawing::FillRectColor(dc, sliderLine, Draw::Color(theme.borderPanel));
    ...
    RECT thumb{ thumbCenter - 5, layout.bottomBar.top + 7, thumbCenter + 5, layout.bottomBar.bottom - 7 };
    GdiDrawing::DrawSharpFrame(dc, thumb, Draw::Color(theme.accent), Draw::Color(theme.accent));
