...
bool EditorMeshAssetInspectorDropHandler::Drop(
    HWND sourceWindow,
    HWND mainWindow,
    int x,
    int y,
    ...
    if (hit.section != InspectorSectionId::MeshRenderer
        || (hit.property != InspectorPropertyId::MeshRendererMesh && hit.property != InspectorPropertyId::MeshRendererMeshPicker)) {
        return false;
    }
    return sceneContext.SetMeshRendererMeshAsset(entity, assetId);
}
