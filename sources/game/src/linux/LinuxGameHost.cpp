    ...
    int Run(std::uint32_t frameLimit) {
        ...
        while (running) {
            while (XPending(display_) > 0) {
                ...
                if (XFilterEvent(&event, window_) != 0) {
                    continue;
                }
            ...
            if (input != nullptr) {
                ...
                if (event.type == KeyPress && inputContext_ != nullptr) {
                    ...
                    if (byteCount > 0 &&
                        ...
                        const char* bytes = overflow.empty()
                            ? utf8.data()
                            : overflow.data();
                        std::array<char32_t,
                            kb::input::InputDeviceState::kMaxTextInputCodePoints>
                            decoded{};
                        ...
                        if (result.wellFormed) {
                            static_cast<void>(input->AddTextInput(
                                std::span<const char32_t>{
                                    decoded.data(), result.codePointCount}));
                        }
                    }
                } else {
                    symbol = XLookupKeysym(
                        const_cast<XKeyEvent*>(&event.xkey), 0);
                }
