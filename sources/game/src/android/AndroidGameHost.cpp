...
void AppendAndroidCommittedText(
    ...
    std::array<char32_t,
        kb::input::InputDeviceState::kMaxTextInputCodePoints> decoded{};
...
public:
    ...
    [[nodiscard]] bool Initialize() {
        ...
        GameActivity_setImeEditorInfo(
            app_.activity, TYPE_CLASS_TEXT, IME_ACTION_DONE,
...
private:
    ...
    void DrainInput() noexcept {
        ...
        if (app_.textInputState != 0) {
            AndroidCommittedTextContext context{ .input =
                state != nullptr && focused_ ? state : nullptr };
    ...
    void RenderFrame() {
        ...
        SetImeVisible(focused_ && scene_->UIDocuments().HasFocusedTextInput());
        FinalizeInputFrame();
